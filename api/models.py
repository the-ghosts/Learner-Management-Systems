...
class Teacher (models.Model):
    ...
    def review(self):
        return Course.objects.filer(teacher= self).count()


class Category (models.Model):
    title = models.CharField(max_length=100)
    image = models.FileField(upload_to= "course-file", default= "default.jpg", blank=True, null= True)
...
class Course(models.Model):
    ...
    platform_status = models.CharField(choices=PLATFORM_STATUS, default="Review", max_length=100)
    featured = models.BooleanField(default=False)
    course_id = ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    slug = models.SlugField(unique=True, null=True, blank=True)
...
class Variant(models.Model):
    ...
    variant_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class VariantItem(models.Model):
    ...
    variant_item_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.file:
            ...
            super().save(updated_field=['content_duration'])

class Question_Answer(models.Model):
    course=models.ForeignKey(Course, on_delete=models.CASCADE)
    user= models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    title= models.CharField(max_length=500)
    qa_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class Question_Answer_Message(models.Model):
    ...
    qam_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    qa_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class Cart(models.Model):
    ...
    cart_id= ShortUUIDField(length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class CartOrder(models.Model):
    ...
    order_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class Certificate(models.Model):
    course=models.ForeignKey(Course, on_delete=models.CASCADE)
    user= models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    certificate_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class EnrolledCourse(models.Model):
    ...
    order_id= models.ForeignKey(CartOrderItem, on_delete=models.CASCADE)
    enrollment_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
    ...
    def completed_lesson(self):
        return CompletedCourse.objects.filter(course=self.course, user=self.user)
...
class Note(models.Model):
    ...
    note_id= ShortUUIDField(unique=True, length= 6, max_length = 10, alphabet="1234567890")
    date= models.DateTimeField(default=timezone.now)
...
class Review(models.Model):
    ...
    def profile(self):
        return Profile.objects.filter.get(user=self.user)
...
class Coupon(models.Model):
    ...
    discount= models.IntegerField(max_length=1)
    active= models.BooleanField(default=False)
    date= models.DateTimeField(default=timezone.now)
